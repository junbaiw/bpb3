...
#try:
    ...
    optional.add_argument("--sep_string_cluster4pwm",help="string used to separate defintation of clustered PWMs suchas quality_assessed~NAME~ or uncertain_pwms~NAME~"
                                         "default is ~ ", type=str, metavar="NAME", default='~')
    optional.add_argument("--initial_string2clusteredPWMs", help="string start for clustered PWMs such as quality_assessed~NAME~, default= quality_assessed", 
                                         type=str, metavar="", default="quality_assessed_")
    optional.add_argument("--notSepString4cluster", help="is string used to separate of clustered PWMs such as quality_assessed~NAME~, default =True ", action="store_false")
...
def run(args):
 ...
 if tp_df.shape[0]>0:
   ...
   for i in range(len(res_data[0])):
     ...
     if not pwm_name.startswith(prefix_str2clustered_pwms):
       if args.notSepString4cluster:
          tag0= pwm_name.split(str_separate)[1]
       else:
          tag0=pwm_name
...
if __name__=='__main__':
  args=my_parser(argparse.ArgumentParser('python filter_results_by_gene_expression_cluster4pwm.py')).parse_args()
  run(args)
